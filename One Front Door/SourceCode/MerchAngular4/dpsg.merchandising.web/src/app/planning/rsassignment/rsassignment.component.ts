...
import {
  HeadernavService
} from '../../services/headernav.service';
import {
  MerchGroup
...
export class RsassignmentComponent implements OnInit {
 ...
 nextDay() {
        ...
        let _selectedWeekDay:number = this.selectedWeekDay;

        if (this.selectedWeekDay == 7) {
            _selectedWeekDay = 0;
        } else {
            _selectedWeekDay = this.selectedWeekDay + 1;
        }
     
     this.setSelectedWeekDay(_selectedWeekDay) ;
