...
import {StoreServiceInput, StoreService, UserMerchGroupInput, UserMerchGroup, ImageInput, ImageDetail, ImageURLInput} from './MerchReport';
...
@Component({
   ...
   // directives: [Header, Pagination, GlobalSearch, CsvExport, DatePicker, MultiselectDropdown, SpinnerComponent],
    ...
    getEachImageURL() {
        ...
        for (var i = 0; i < this.imgDetail.length; i++) {
            ...
            this.imgURLInput = new ImageURLInput(
                this.imgDetail[i].RelativeURL,
                this.imgDetail[i].AbsoluteURL,
                this.imgDetail[i].Container,
                this.imgDetail[i].StorageAccount,
                this.imgDetail[i].AccessLevel,
                this.imgDetail[i].ConnectionString
            );
