...
var StoreserviceComponent = (function () {
    ...
    StoreserviceComponent.prototype.getImageDetails = function (imageBlobID, isSign) {
        this.imgURLList = [];
        if (imageBlobID) {
            //  //Call WebAPI to get the details
            this.dispService.set(this._webapi + 'api/Merc/GetImageDetailByBlobID/');
            this.imgInput = new MerchReport_1.ImageInput(imageBlobID, isSign);
    ...
    StoreserviceComponent.prototype.getEachImageURL = function () {
        ...
        for (var i = 0; i < this.imgDetail.length; i++) {
            ...
            this.imgURLInput = new MerchReport_1.ImageURLInput(this.imgDetail[i].RelativeURL, this.imgDetail[i].AbsoluteURL, this.imgDetail[i].Container, this.imgDetail[i].StorageAccount, this.imgDetail[i].AccessLevel, this.imgDetail[i].ConnectionString);
    ...
    StoreserviceComponent.prototype.getImageURL = function (inputdata) {
        ...
        this.dispService.post(JSON.stringify(inputdata), true)
            ...
            _this.imgURLList.push(d.Response);
            _this.lgModal.show();
        }, function (error) {
