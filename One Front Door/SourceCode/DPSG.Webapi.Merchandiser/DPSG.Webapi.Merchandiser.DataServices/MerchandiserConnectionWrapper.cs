...
{
    public class MerchandiserConnectionWrapper : IDisposable
    {
        MerchandiserConnection _connection;

        protected MerchandiserConnection dbConnection
        {
        ...
        {
            _connection = new MerchandiserConnection();
        }
        ...
        internal SDMReader ExecuteReader(string commandName, SqlParameter[] paramArray)
        {
            var cmd = this.dbConnection.Database.Connection.CreateCommand();
            cmd.CommandText = commandName;
            cmd.CommandType = System.Data.CommandType.StoredProcedure;
            ...
            if (paramArray != null)
                cmd.Parameters.AddRange(paramArray);
            var reader = cmd.ExecuteReader();
            SDMReader sdmReader = new SDMReader(reader, ((System.Data.Entity.Infrastructure.IObjectContextAdapter)dbConnection).ObjectContext);
            return sdmReader;
        }

        internal async Task<SDMReader> ExecuteReaderAsync(string commandName, SqlParameter[] paramArray)
        {
            var cmd = this.dbConnection.Database.Connection.CreateCommand();
            cmd.CommandText = commandName;
            cmd.CommandType = System.Data.CommandType.StoredProcedure;
    ...
    internal class SDMReader
    {
        public SDMReader(DbDataReader rdr, System.Data.Entity.Core.Objects.ObjectContext ctxt)
        {
            reader = rdr;
            context = ctxt;
        }
