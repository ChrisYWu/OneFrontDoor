...
SELECT DISTINCT   
        ...
        lh.BranchID AS BranchID, lh.AreaID, lh.SAPAreaID, lh.AreaName, lc.ChannelID, lc.SuperChannelID  
FROM            SAP.Account AS a INNER JOIN  
                ...
                MView.LocationHier AS lh ON br.BranchID = lh.BranchID  INNER JOIN
                MView.LocationChannel As lc ON lh.BranchID = lc.BranchID
WHERE        (a.Active = 1)  
GO
...
AS  
Select Distinct ch.*, lh.*  , sch.SuperChannelName, sch.SAPSuperChannelID
From SAP.Account a  
Join SAP.Branch br on br.BranchID = a.BranchID  
Join SAP.Channel ch on a.ChannelID = ch.ChannelID  
Join SAP.SuperChannel sch on ch.SuperChannelID = sch.SuperChannelID
