-- Provided by Vijay, executed on 4pm 20130610

USE [Portal_Data]
GO

/****** Object:  StoredProcedure [PlayBook].[pInsertPromotionRanking]    Script Date: 6/10/2013 3:34:25 PM ******/
SET ANSI_NULLS ON
GO
...
-- Description: This procedure inserts the data in ranking table based on the start date and end date  
--Example : EXEC [PlayBook].[pInsertPromotionRanking] '124', '2013-05-22', '2013-06-04'  
-- =============================================         
ALTER PROCEDURE [PlayBook].[pInsertPromotionRanking]   
@PromotionID int,  
@StartDate Date,   
@EndDate Date  
...
AS  
BEGIN  
 DECLARE @Day varchar(25) = DATENAME(dw,@StartDate)  
 DECLARE @tempStartDate DATE = @StartDate  
 DECLARE @tempEndDate DATE = @StartDate  
 
 --Delete the Existing Record
 Delete From PlayBook.PromotionRank
 where PromotionID = @PromotionID
 
   
 While (@EndDate >= @tempEndDate)  
 BEGIN  
  if ( @Day = 'Monday')  -- If first day is monday then continue adding entry else get the date with day as monday
   BEGIN  
    SET @tempEndDate = DATEADD(DAY, 6, @tempStartDate)  
    INSERT INTO PlayBook.PromotionRank (PromotionID,PromotionWeekStart,PromotionWeekEnd,Rank)  
    VALUES (@PromotionID,@tempStartDate,@tempEndDate,100)  
    SET @tempStartDate = DATEADD(DAY, 1, @tempEndDate)  
   END  
  ELSE  
   BEGIN  
    while (@Day != 'Monday')  
     BEGIN  
      ...
      SET @Day = datename(dw,@tempStartDate)  
     END  
   END   
 END  
END  
...
USE [Portal_Data]
GO
/****** Object:  StoredProcedure [PlayBook].[pGetRoutePromotions]    Script Date: 6/10/2013 9:48:42 AM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
exec Playbook.pGetRoutePromotions @routeNumber = 100386010

      , @promotionStartDate = '06-27-2013'
...
And PromotionstartDate <= GetDate()
 -- will get all the promotions while they are either on the fly or being planned for the future
--And GetDate() between Getdate() and '06-20-2013'
