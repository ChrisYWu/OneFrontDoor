...
If Exists (Select * From sys.procedures Where object_id = object_id('BCMyday.pGetBCPromotionsForGSN8'))
	Drop Proc BCMyday.pGetBCPromotionsForGSN8
Go
...
/*
exec BCMyday.pGetBCPromotionsForGSN8 @GSN = 'BUTJX004', @EnableLogging = 1

exec BCMyday.pGetBCPromotionsForGSN8 @GSN = 'BUTJX004', @Debug = 1, @EnableLogging = 1

exec BCMyday.pGetBCPromotionsForGSN8 @GSN = 'WATBX001', @Debug = 1

exec BCMyday.pGetBCPromotionsForGSN8 @GSN = 'WATBX001', @EnableLogging = 1

exec BCMyday.pGetBCPromotionsForGSN8 @GSN = 'ALBDX003', @EnableLogging = 1

exec BCMyday.pGetBCPromotionsForGSN8 @GSN = 'BUTJX004', @EnableLogging = 1

exec BCMyday.pGetBCPromotionsForGSN8 @GSN = 'WATBX001', @Debug = 1, @EnableLogging = 1

*/

Go

Create Proc BCMyday.pGetBCPromotionsForGSN8
(
	@GSN Varchar(50), 
...
BEGIN
	...
	SELECT pa.PromotionID 'PromotionID'
		...
		,(case when AttachmentName like '%.%'
              then reverse(left(reverse(AttachmentName), charindex('.', reverse(AttachmentName)) - 1))
              else ''
         end) as Extension
		,AttachmentSize 'Size'
		...
		,Case When GetDate() Between PromotionStartDate And PromotionEndDate Then 'Yes' Else 'No' End CurrentPromotion
	...
	Join Playbook.RetailPromotion rp on pids.PromotionID = rp.PromotionID
