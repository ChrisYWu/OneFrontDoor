...
If Exists (Select * From sys.procedures Where object_id = object_id('Playbook.pDataAnalysisForNitin'))
	Drop Proc Playbook.pDataAnalysisForNitin
Go
...
/*
--- Irving ---
exec Playbook.pDataAnalysisForNitin @BranchIDs = 161, @Date = '2015-1-1'
exec Playbook.pDataAnalysisForNitin @BranchIDs = '118,119,120,121,122,123,124,125,126,127,128,129,130,131,132,133,134,135,136,137,138,139,140,141,142,143,144,154,155,165,167,168,169', @Date = '2015-1-1'
...
--- West BU ---
exec Playbook.pDataAnalysisForNitin @Debug = 1, @Date = '2015-1-1', @BranchIDs = '118,119,120,121,122,123,124,125,126,127,128,129,130,131,132,133,134,135,136,137,138,139,140,141,142,143,144,154,155,165,167,168,169'

*/
Create Proc Playbook.pDataAnalysisForNitin
(
...
Begin
	...
	Select Case When cg.MSTRChainGroupID Like 'N%' Or temp.NationalChainID <> 62 Then 'National'
				When temp.RegionalChainID <> 242 Then 'Regional' Else 'Local' End ChainTypeIdentifier, *
	From Playbook.ChainGroup cg
	Join
	(
		Select Distinct mstr.ChainGroupID, cs.LocalChainID, cs.RegionalChainID, cs.NationalChainID
		From @PromoAccountHier pa
		Join Playbook.ChainGroupLocalChain mstr on pa.LocalChainID = mstr.LocalChainID
		Join Processing.tChainSeeking cs on mstr.LocalChainID = cs.LocalChainID
		Union
		Select Distinct mstr.ChainGroupID, null, cs.RegionalChainID, cs.NationalChainID
		From @PromoAccountHier pa
		Join Playbook.ChainGroupRegionalChain mstr on pa.RegionalChainID = mstr.RegionalChainID
		Join Processing.tChainSeeking cs on mstr.RegionalChainID = cs.RegionalChainID
		Union
		Select Distinct mstr.ChainGroupID, null, null, mstr.NationalChainID
		From @PromoAccountHier pa
		Join Playbook.ChainGroupNationalChain mstr on pa.NationalChainID = mstr.NationalChainID
	) temp on cg.ChainGroupID = temp.ChainGroupID
	Order By ChainTypeIdentifier, ChainGroupName
